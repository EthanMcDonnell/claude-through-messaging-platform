...
Polls iMessage for self-sent messages and routes them to Claude CLI.
"""
...
from watcher import current_db_timestamp, fetch_new_messages, get_db_connection
...
def _validate_config(config: dict) -> None:
    """Fail fast on startup if config is invalid or project paths are unsafe."""
    for project in config.get("projects", []):
        path = Path(project.get("path", "")).resolve()
        if not path.is_dir():
            raise ValueError(f"Project '{project['name']}' path does not exist: {path}")
        # Ensure the resolved path didn't escape via symlinks to somewhere unexpected
...
def run(config: dict, state: State, dry_run: bool = False) -> None:
    poll_interval = config.get("poll_interval", 3)
    allowed_sender = config.get("allowed_sender", "")
    self_chat_id = config.get("self_chat_id", allowed_sender)
    ...
    logger.info("Starting bridge. Project: %s | Chat: %s", state.current_project, self_chat_id)
    if dry_run:
        logger.info("DRY RUN MODE — messages will not be sent")

    conn = get_db_connection()

    # Initialize cursor to now (don't replay old messages on startup)
    ...
    while True:
        ...
        for msg in new_messages:
            ...
            # Convert iMessage nanosecond timestamp to Unix for cursor advancement
...
def main():
    parser = argparse.ArgumentParser(description="iMessage ↔ Claude Bridge")
    parser.add_argument(
        "--config",
